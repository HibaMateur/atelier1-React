...
class ProductList extends Component {
  ...
  handleClick() {
    this.setState((prevState) => {
      return { const :index = Number(e.currentTarget.value),
        count: this.setState(this.state.count.map(c, i)=> { return i === index ? C+1: c}) };
    });
  }
        ...
        {ProductData.map((productDetail, index) => {
          return (
            <ProductFrame key={index}>
              ...
              <ProductInfoWrapper>
              <input type="button" onClick={this.handleClick} value={index} name="like"/>
              </ProductInfoWrapper>
            </ProductFrame>
          );
        })}
